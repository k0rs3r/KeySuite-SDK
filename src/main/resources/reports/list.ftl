...
<!--<span class="row" style="font-weight:bold">
    ...
    <span class="col-sm-12">
			<span class="col-sm-4">
				<a href="query/${report.qt}${ report.form???then('?form','') }">${report.title}</a>
			</span>
			<span class="col-sm-4" style="font-size:larger" >
				<a title="esegui" href="${context}/query/${report.qt}"><i class="glyphicon glyphicon-play-circle"></i></a>
				<#if (report.form?has_content) >
                    <a title="mostra form" href="${context}/query/${report.qt}?form"><i class="glyphicon glyphicon-list-alt"></i></a>
				</#if>
			</span>
			<span class="col-sm-4">
			</span>
