...
import it.kdm.doctoolkit.services.AuthenticationService;
...
//		lotlSource.setCertificateSource(officialJournalContentKeyStore());
    ...
    public String generateHtmlReportBS3(){
        try {
            return SimpleReportFacade.newFacade().generateHtmlBootstrap3Report(this.simpleReport);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private void validateNested(Date verificationDate){
        ...
        for( DSSDocument docs : originalDocuments){
            try{
                SignatureValidator val = new SignatureValidator(docs.openStream());
